<script lang="ts">
	export let color = 'fuchsia';
	export let shadow_color = 'rgba(255, 0, 130, 0.5)';
	export let active_shadow_color = 'rgba(255, 0, 130, 0.2)';
	export let on_click:
		| ((e: MouseEvent & { currentTarget: EventTarget & HTMLButtonElement }) => void)
		| null = null;
...
<button
    ...
    --active-shadow-color: {active_shadow_color};
    "
	on:click={(event) => {
		if (on_click != null) {
			on_click(event);
...
<style>
	button {
		...
		box-shadow: 0 2px 25px var(--shadow_color);
		...
		&:active {
			...
			box-shadow: 0 2px 25px var(--active-shadow-color);
		}
